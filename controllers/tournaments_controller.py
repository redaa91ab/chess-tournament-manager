...
class TournamentsController:
        ...
        while True:
            ...
            if user_choice == 1:
                ...
                if tournament:
                    if tournament.state == "finished":
                        self.view.show_message(
                            "This tournament is finished. You can't add players."
                        )
                    else:
                ...
                if tournament:
                    if tournament.state == "finished":
                        self.view.show_message(
                            "This tournament is finished. You can't play a tournament."
                        )
                    elif len(tournament.players) < 4:
                        self.view.show_message(
                            f"You need at least 4 players to play a tournament."
                            f"Current number of players = {len(tournament.players)}"
                        )
                    elif len(tournament.players) % 2 != 0:
                        self.view.show_message(
                            f"You need a even number of players to play."
                            f"Current number of players = {len(tournament.players)}"
                        )
                    else:
    ...
    def add_player_tournament(self, tournament):
        """
        1) Retrieve player data from national_chess_id input
        2) Create a PlayerTournament instance and save it in the players list in the tournament
        """
        while True:
            national_chess_id = (
                self.players_controller._get_valid_national_chess_id()
            )
            ...
            if player is None:
                ...
                if user_choice == 1:
                    ...
                    player = self.players_controller.add_player(
                        national_chess_id
                    )
                    ...
                    tournament.save_json()
                    self.view.show_message(
                        f"\n{player.name} {player.surname} ({player.national_chess_id})"
                        f"was successfully added to the tournament !"
                    )
                    break
            elif player:
                nid_player = player.national_chess_id
                nid_players_tournament = [
                    player.national_chess_id for player in tournament.players
                ]
                if nid_player in nid_players_tournament:
                    self.view.show_message(
                        f"\n{player.name} {player.surname} ({player.national_chess_id})"
                        f"is already saved in the tournament."
                    )
                    break
                else:
                    tournament.players.append(
    ...
    def play_tournament(self, tournament):
        """
        Display play_tournament menu, then directs to :
        Create a new round, update the actual round or finish the tournament
        """
    ...
    def finish_tournament(self, tournament):
        """
        Verify that the tournament can be finished, then :
        Change the state to "finished" and display the results of the tournament
        """
    ...
    def reports_tournament(self, tournament):
        """
        Menu of one tournament reports, offer 3 options :
        Display all the players of the tournament, display the history of the tournament, or going back
        """
        while True:
...
class RoundController:
    ...
    def create_round_menu(self, tournament):
        ...
        while True:
            ...
            if len(rounds) > 0:
                ...
                if actual_round.state == "in_progress":
                    self.view.show_message(
                        "Please update the current round before to create a new one"
                    )
                    break
            if tournament.state == "finished":
                self.view.show_message(
                    "The tournament is finished. You can't create a new round)"
                )
                break
            ...
            for player in players:
                ...
                if len(players) > len(previous_opponents):
                    players = [
                        player
                        for player in players
                        if player not in previous_opponents
                    ]
                opponent = players[0]
    ...
    def update_results_games(self, tournament):
        ...
        while True:
            ...
            if len(rounds) == 0:
                self.view.show_message(
                    "Please create a round before to update"
                )
                break
            else:
                actual_round = rounds[current_round - 1]
                if actual_round.state == "finished":
                    self.view.show_message(
                        "The round is already updated, please create a new one"
                    )
                    break
                elif actual_round.state == "in_progress":
                    for game in actual_round.games_list:
                        winner = self.view.update_game_result(
                            actual_round.name, game
                        )
                    ...
                    if current_round == tournament.number_of_rounds:
                        TournamentsController(self.view).finish_tournament(
                            tournament
                        )
                    tournament.save_json()
                    break
