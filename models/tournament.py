...
class Tournament:
    ...
    def save_json(self):
        ...
        tournament_table.upsert(
            self.serialize(), (Query()["tournament_id"] == self.tournament_id)
        )
    ...
    def deserialize(cls, tournament_serialized):
        ...
        players = [
            PlayerTournament.deserialize(player)
            for player in tournament_serialized["players"]
        ]
        rounds = [
            Round.deserialize(round, players)
    ...
    def deserialize_all_tournaments(cls):
        ...
        tournaments = [
            Tournament.deserialize(tournament)
    ...
    def get_players_sorted(self):
        ...
        players_list_sorted = sorted(
            players_list, key=lambda player: player.total_points
        )
    ...
    def get_previous_opponents(self, player):
        ...
        for round in self.rounds:
            previous_opponents_round = round.get_previous_opponents_round(
                player
            )
...
class Round:
    ...
    def __init__(
        self, name, games_list, state, start_date=None, end_date=None
    ):
        """
    ...
    def deserialize(cls, round_serialized, players):
        ...
        games_list = [
            Game.deserialize(game, players)
            for game in round_serialized["games_list"]
        ]
        state = round_serialized["state"]
        start_date = round_serialized["start_date"]
        end_date = round_serialized["end_date"]

        round_deserialized = Round(
            name, games_list, state, start_date, end_date
        )
...
class Game:
    ...
    def deserialize(cls, game_serialized, players):
        ...
        player1_tournament = PlayerTournament.deserialize_by_nid(
            player1_national_chess_id, players
        )
        score_player1 = game_serialized[0][1]

        player2_national_chess_id = game_serialized[1][0]
        player2_tournament = PlayerTournament.deserialize_by_nid(
            player2_national_chess_id, players
        )
        score_player2 = game_serialized[1][1]

        game_deserialized = Game(
            player1_tournament,
            score_player1,
            player2_tournament,
            score_player2,
...
class PlayerTournament(Player):
    ...
    def __init__(
        self, national_chess_id, name, surname, birthdate, total_points
    ):
        """
    ...
    def serialize(self):
        ...
        serialized_data = [self.national_chess_id, self.total_points]
        return serialized_data
    ...
    def deserialize(cls, player_tournament_serialized):
        ...
        player_tournament_deserialized = PlayerTournament(
    ...
    def deserialize_by_nid(cls, national_chess_id, players: list):
        ...
        player_tournament_deserialized = next(
            (
                player
